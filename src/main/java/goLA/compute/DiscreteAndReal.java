...
public class DiscreteAndReal implements QueryProcessor {
    ...
    public List<Trajectory> query(Query query, List<Trajectory> trh) {
        ...
        List<Trajectory> trajectories = trh
                .stream()
                .filter(t ->
                        t.isResult() || DiscreteFrechetDistance.decisionDP(query.getQueryTrajectory(), t, query.getDistance()) || FrechetDistance.decisionDP(query.getQueryTrajectory(), t, query.getDistance())
                )
